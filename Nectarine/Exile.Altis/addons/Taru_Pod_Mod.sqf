/*
    ...
    Epoch Taru Pod's mod script by Halv - inspired by XENO Taru Pod Mod
    Copyright (C) 2015  Halvhjearne

    /////////////////////////////////////////
    Edited and modded for Exilemod by SLB2k11
    /////////////////////////////////////////
*/
        ...
        HALV_fnc_parapod = compileFinal "
            ...
            if(!isNull _player && !isNull _para)then{
                if(_para isKindOf 'ParachuteBase' || _para isKindOf 'Pod_Heli_Transport_04_base_F')then{
                    _para call ExileServer_object_vehicle_database_update;
                };
            };
        ";
        HALV_fnc_savepod = compileFinal "
            ...
            if (!isNull _pod && !isNull _player)then{
                if(_pod isKindOf 'Pod_Heli_Transport_04_base_F' && _pod getVariable ['VEHICLE_SLOT','ABORT'] != 'ABORT')then{
                    ...
                    _pod call ExileServer_object_vehicle_database_update;
                };
            };
        ";
...
if(hasInterface && !isDedicated)then{
    HALV_attachTarupods = {
        ...
        _attribs = switch (typeOf _pod)do{
            ...
            case "Land_Pod_Heli_Transport_04_bench_black_F":{[[0,0,-1.2],680]};
            case "Land_Pod_Heli_Transport_04_covered_black_F":{[[0,-1,-0.82],1413]};
            case "Land_Pod_Heli_Transport_04_medevac_black_F":{[[-0.15,-1.08,-0.87],1321]};
            case "Land_Pod_Heli_Transport_04_box_black_F":{[[-0.1,-1.08,-1.07],1270]};
            case "Land_Pod_Heli_Transport_04_fuel_black_F":{[[-0.025,-0.5,-1.22],13311]};
            case "Land_Pod_Heli_Transport_04_repair_black_F":{[[-0.1,-1.08,-1.07],1270]};
            case "Land_Pod_Heli_Transport_04_ammo_black_F":{[[-0.1,-1.08,-1.07],1270]};
...
//workaround for ropes not being disabled with above command before ropes has been attached once
    ...
    HALV_detachTarupods = {
        ...
        _attribs = switch (typeOf _pod)do{
            ...
            case "Land_Pod_Heli_Transport_04_bench_black_F":{680};
            case "Land_Pod_Heli_Transport_04_covered_black_F":{1413};
            case "Land_Pod_Heli_Transport_04_medevac_black_F":{1321};
            case "Land_Pod_Heli_Transport_04_box_black_F":{1270};
            case "Land_Pod_Heli_Transport_04_fuel_black_F":{13311};
            case "Land_Pod_Heli_Transport_04_repair_black_F":{1270};
            case "Land_Pod_Heli_Transport_04_ammo_black_F":{1270};
            default{1270};
        };
        _taruweight = (weightRTD _heli)select 3;
    ...
    while{alive player}do{
        if !(player isEqualTo vehicle player)then{
            ...
            _isTaru = _vehicle isKindOf "Exile_Chopper_Taru_Transport_Black" OR _vehicle isKindOf "Exile_Chopper_Taru_Transport_CSAT";
            if(_isTaru)then{
